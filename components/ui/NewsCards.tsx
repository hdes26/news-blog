...
export const NewsCard: React.FC<{ data: News }> = ({ data }) => {
  ...
  return (
    <Link href={data.title} className={`${colSpan} rounded-3xl overflow-hidden cursor-pointer hover:scale-[103%] transition duration-300 ease-in-out`}>
      <div className='relative w-full h-full p-4 md:p-8 bg-[#00000077]'>
        <Image src='/giphy.gif' priority width={0} height={0} alt='rat' className='absolute w-full h-full object-cover object-center top-0 left-0 -z-10' />
        ...
        <div className='w-full h-full flex flex-col justify-between'>
          ...
          <div>
            ...
            <p className='text-white text-sm md:text-md lg:text-2xl'>My Github profile, where I upload my projects.</p>
          </div>
        </div>
      </div>
    </Link>
